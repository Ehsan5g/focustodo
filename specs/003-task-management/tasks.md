...
- [x] T021 [P] [US3] Write the failing component tests for the status toggle in `tests/component/TaskStatusToggle.test.tsx`: optimistic flip on click before the action resolves; failure → rollback + friendly error + resync (FR-009, D3); label reflects the target state (complete vs reopen) — ✅ 4 tests passing (109 total): target-state labels, optimistic flip before resolution, failure rollback + friendly error + resync (`onUpdated` fires), D8 re-fire gate
- [x] T022 [P] [US3] Write the failing E2E toggle scenarios in `tests/e2e/tasks.spec.ts`: complete a TODO task directly (forward skip — US3 S1, clarified lifecycle); reopen COMPLETED → TODO (S6, FR-008); rapid toggling settles consistent; rollback via request interception — abort `setTaskStatus` mid-flight, assert the UI rolled back + an understandable error shows + a reload matches the server state (S7, SC-004, D12) — ✅ all 30 E2E tests green (34 s, workers:1); DB cross-checks scoped to each test's registered user (hermetic vs parallel/stale rows); exposed + fixed a real D8 gap (see T024)

### Implementation for User Story 3

- [x] T023 [US3] Implement the `setTaskStatus` server action in `src/server/actions/task-actions.ts` per contracts/server-actions.md — five steps: (1) validate via `taskIdSchema` + `statusSchema`; (2) `requireSession()`; (3) authorize — scoped lookup; foreign/unknown id → the SAME friendly failure (D9); (4) execute — same-status no-op → success; the matrix consulted via the shared `validateTransition`, backward pair → `validation_error` on `status` (D2); (5) return `TaskActionResult` + `revalidatePath("/")` (depends on T007, T009) — ✅ implemented + 3 integration tests
- [x] T024 [US3] Create `src/features/tasks/TaskStatusToggle.tsx` making T021 pass: `useOptimistic` + `useTransition` wired to the action (D3), pending-gated against double-fires (D8), integrated into TaskItem (depends on T021, T023) — ✅ implemented (useState-based optimistic + single-flight ref, settled to the action's returned status until the refreshed prop confirms it); failure paths also resync via `onUpdated` (D3); E2E exposed that a stale prop + optimistic label let a second click recompute the OLD target (same-status no-op) — fixed by deriving the target from the displayed status
- [x] T025 [US3] Validate US3 independently: T021/T022 green incl. the interception rollback; quickstart S6 (toggle steps — its backward-pick step lands with US4's edit surface, T027/T029) + S7 manual pass; SC-004 verified; gates green; commit — ✅ vitest 109/109, lint/format:check/typecheck clean, E2E 30/30 incl. interception rollback (SC-004); S6 toggle steps verified via US3 S1/S6, rapid-toggle D12 scenario green
